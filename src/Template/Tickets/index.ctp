    ...
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                ...
                <th scope="col"><?= $this->Paginator->sort('visitor_id') ?></th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?= h($ticket->confirmed) ?></td>
                <td><?= h($ticket->created) ?></td>
                <td><?= h($ticket->modified) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $ticket->visitor_id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $ticket->visitor_id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $ticket->visitor_id], ['confirm' => __('Are you sure you want to delete # {0}?', $ticket->visitor_id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
